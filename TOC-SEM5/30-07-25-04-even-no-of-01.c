...
bool q1(char s[],int i){
	...
	else if (s[i]=='0') return q0(s,++i);
	else return q3(s,++i);
}

bool q2(char s[],int i){
	if (s[i]=='\0') return 0; //reject state
	else if (s[i]=='0') return q3(s,++i);
	else return q0(s,++i);
}

bool q3(char s[],int i){
	if (s[i]=='\0') return 0; //reject state
	else if (s[i]=='0') return q2(s,++i);
	else return q1(s,++i);
}
...
int main(){
	...
	printf("The given binary number %s %s contain even number of 0s & 1s ",s,(q0(s,0)?"does":"doesn't"));
	return 0; 
}